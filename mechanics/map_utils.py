...
def make_bsp(game_map, player, entities):
    ...
    down_stairs = Entity(last_room_x, last_room_y, '>', (255, 255, 255),
                         'Escadas', render_order=RenderOrder.STAIRS, stairs=stairs_component)
    entities.append(down_stairs)


def next_floor(player, message_log, dungeon_level, constants):
    game_map = GameMap(constants['MAP_WIDTH'],
                       constants['MAP_HEIGHT'], dungeon_level)
    entities = [player]
    ...
    message_log.add_message(Message(
        'Voce para um momento para descansar, e recuperar suas forcas.', colors.light_violet))
    ...
    for i in range(number_of_monsters):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            ...
            if monster_choice == 'apprentice':
                ...
                monster = Entity(x, y, 'a', colors.white, 'Aprendiz', blocks=True, render_order=RenderOrder.ACTOR,
    ...
    for i in range(number_of_items):
        ...
        if not any([entity for entity in entities if entity.x == x and entity.y == y]):
            ...
            if item_choice == 'healing_potion':
                ...
                item = Entity(x, y, '!', colors.violet, 'Pocao de Cura',
                              render_order=RenderOrder.ITEM, item=item_component)

            elif item_choice == 'sword':
                equippable_component = Equippable(
                    EquipmentSlots.MAIN_HAND, power_bonus=3)
                item = Entity(x, y, '/', colors.sky, 'Espada Curta',
                              equippable=equippable_component)

            elif item_choice == 'shield':
                equippable_component = Equippable(
                    EquipmentSlots.OFF_HAND, defense_bonus=4)
                item = Entity(x, y, '[', colors.darker_orange,
                              'Escudo do Homem Pobre', equippable=equippable_component)

            elif item_choice == 'helmet':
                equippable_component = Equippable(
                    EquipmentSlots.HEAD, defense_bonus=1)
                item = Entity(x, y, '^', colors.darker_green,
                              'Elmo', equippable=equippable_component)

            elif item_choice == 'calca':
                equippable_component = Equippable(
                    EquipmentSlots.CALCA, max_hp_bonus=5)
                item = Entity(x, y, 'V', colors.yellow, 'Calca',
                              equippable=equippable_component)

            elif item_choice == 'peitoral':
                equippable_component = Equippable(
                    EquipmentSlots.PEITORAL, defense_bonus=3)
                item = Entity(x, y, 'H', colors.light_pink,
                              'Peitoral', equippable=equippable_component)

            elif item_choice == 'bota':
                equippable_component = Equippable(
                    EquipmentSlots.BOTA, defense_bonus=1)
                item = Entity(x, y, 'p', colors.black, 'Bota',
                              equippable=equippable_component)
                ...
                item = Entity(x, y, '#', colors.red, 'Pergaminho da Bola de Fogo',
                              render_order=RenderOrder.ITEM, item=item_component)
                ...
                item = Entity(x, y, '#', colors.light_pink, 'Pergaminho da Confusao',
                              render_order=RenderOrder.ITEM, item=item_component)

            else:
                item_component = Item(
                    use_function=cast_lightning, damage=40, maximum_range=5)
                item = Entity(x, y, '#', colors.yellow, 'Pergaminho de Raio',
                              render_order=RenderOrder.ITEM, item=item_component)
